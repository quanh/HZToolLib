//
//  MutiSectionCollectionViewFlowLayout.swift
//  GifEditor
//
//  Created by quanhai on 2022/3/19.
//
...
@objc protocol MutiSectionCollectionViewFlowLayoutDelegate {
  // required
  /// collectionItem高度
  func heightForRowAtIndexPath(collectionView collection: UICollectionView, layout: MutiSectionCollectionViewFlowLayout, indexPath: IndexPath, itemWidth: CGFloat) -> CGFloat
  
  // optional
  /// 每个section 列数（默认2列）
  @objc optional func columnNumber(collectionView collection: UICollectionView, layout: MutiSectionCollectionViewFlowLayout, section: Int) -> Int
  
  /// header高度（默认为0）
  @objc optional func referenceSizeForHeader(collectionView collection: UICollectionView, layout: MutiSectionCollectionViewFlowLayout, section: Int) -> CGSize
  
  /// footer高度（默认为0）
  @objc optional func referenceSizeForFooter(collectionView collection: UICollectionView, layout: MutiSectionCollectionViewFlowLayout, section: Int) -> CGSize
  
  /// 每个section 边距（默认为0）
  @objc optional func insetForSection(collectionView collection: UICollectionView, layout: MutiSectionCollectionViewFlowLayout, section: Int) -> UIEdgeInsets
  
  /// 每个section item上下间距（默认为0）
  @objc optional func lineSpacing(collectionView collection: UICollectionView, layout: MutiSectionCollectionViewFlowLayout, section: Int) -> CGFloat
  
  /// 每个section item左右间距（默认为0）
  @objc optional func interitemSpacing(collectionView collection: UICollectionView, layout: MutiSectionCollectionViewFlowLayout, section: Int) -> CGFloat
  
  /// section头部header与上个section尾部footer间距（默认为0）
  @objc optional func spacingWithLastSection(collectionView collection: UICollectionView, layout: MutiSectionCollectionViewFlowLayout, section: Int) -> CGFloat
}


class MutiSectionCollectionViewFlowLayout: UICollectionViewFlowLayout{
	weak var delegate: MutiSectionCollectionViewFlowLayoutDelegate?
